  ...
  float bayer8(vec2 frag) {
    /* The 8x8 Bayer value is the bit-reversed interleave of x and y.  GLSL ES
       1.00 has no bitwise operators, so bits come out by mod/floor and XOR is
       written as a + b - 2ab on 0/1 values.  Exact: every intermediate is a
       small integer in float, and n/64 is exact in binary. */
    float x = floor(mod(frag.x, 8.0));
    float y = floor(mod(frag.y, 8.0));
    float x0 = mod(x, 2.0);
    ...
    float m = 32.0 * (x0 + y0 - 2.0 * x0 * y0) + 16.0 * y0
            +  8.0 * (x1 + y1 - 2.0 * x1 * y1) +  4.0 * y1
            +  2.0 * (x2 + y2 - 2.0 * x2 * y2) +         y2;
    return m / 64.0 - 0.5;
  }
